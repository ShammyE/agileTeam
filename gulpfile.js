...
gulp.task("styles", function() {
  return gulp.src(paths.styles).pipe(styles());
});
...
gulp.task("watch", function() {
  ...
  $.watch(paths.test).pipe($.plumber()).pipe(lintScripts());

  gulp.watch("bower.json", ["bower"]);
});

gulp.task("serve", function(cb) {
  runSequence("clean:tmp", ["lint:scripts"], ["start:client"], "watch", cb);
});
